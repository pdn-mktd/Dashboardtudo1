...
export default function Clients() {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Nome', 'Email', 'Produto Principal', 'Add-ons', 'Qtd Add-ons', 'Status', 'Data Entrada', 'Data Cancel.', 'Motivo Cancel.', 'MRR Total', 'Anotações'];

    const rows = clients.map(client => {
      const mrr = getClientMrr(client);
      const activeAddons = client.client_addons?.filter(a => a.status === 'active') || [];

      // Formatar add-ons detalhados: "Produto1 x2, Produto2 x1"
      const addonsDetail = activeAddons.length > 0
        ? activeAddons.map(a => `${a.products?.name || 'Addon'} x${a.quantity}`).join(', ')
        : '-';
